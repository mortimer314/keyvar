    ...
    <>
      ...
      <div
        ...
        className={` fixed ${customizeContext.language === "fa" ? '-right-1 sm:-right-11 border-t-0 rounded-t-none' : "-left-1 sm:-left-11 border-b-0  rounded-b-none"} bottom-0 top-0 m-auto rotate-90 
              w-fit h-fit bg-white dark:bg-slate-700 select-none 
              shadow-sm text-sm text-gray-400 dark:text-gray-300  
              ...
              `}
      >
        <div className='font-DanaBold hidden sm:block'>
          {
      ...
      >
        ...
        {/* customize sidebar */}
        <div className={`transition-all duration-300 max-w-[455px] h-full   shrink 
          ${isShowCustimization ? "translate-0 bg-white" : customizeContext.language === "fa" ? "translate-x-[455px] bg-white/50" : "translate-x-[-455px] bg-white/50"}`}>
          {/* header */}
          <div className="p-4 border-b border-solid border-gray-200 ">
            <div className="flex justify-between mb-10">
              <div className="">
                <div className="text-gray-700 flex gap-2 items-center pb-2">
                ...
                <h5 className='text-xs font-DanaMedium text-gray-500/80 '>{customizationInfos.subtitle}</h5>
              </div>
              <button className="flex items-start " onClick={showCustomization}>
                <svg className='w-5 h-5 '>
                  <use href='#x-mark'></use>
                </svg>
              </button>
            </div>
            <button className='flex gap-x-2 font-semibold items-center justify-center text-xs w-full py-2 rounded-md border border-slate-300  text-black/80 dark:text-gray-300 bg-slate-100/80 hover:bg-slate-200 transition'>
              <div className="">
          ...
          ${customizeContext.language === "fa" ? "pl-[9px]" : "pr-[9px]"}
          `}>
            {/* theme color */}
